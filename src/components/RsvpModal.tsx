...
export const RsvpModal: React.FC<RsvpModalProps> = ({ isOpen, onClose }) => {
  ...
  const slots = language === 'en' ? [
    'Morning darshan (10:00 AM - 12:00 PM)',
    'Midday maha aarti & prasad (11:30 AM)',
    'Evening diya maha aarti (7:30 PM)',
    'Darshan at any time during the day',
  ] : [
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      ...
      <div className="relative z-10 w-full sm:max-w-lg bg-[#1a0c0f] border-t sm:border border-[#d4af37]/40 rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto shadow-2xl animate-slideUp">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-[#d4af37]/20">
          <div className="flex items-center gap-2">
            ...
            <span className="font-devanagari text-base font-bold text-[#faf5ee]">
              {t('सपरिवार उपस्थिति (RSVP)', 'Family attendance (RSVP)')}
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <p className="font-devanagari text-xs sm:text-sm text-[#e6dcce] mt-3">
          {t('आपकी उपस्थिति हमारे लिए अत्यंत आनंद का विषय होगी। कृपया अपनी सुविधा अनुसार समय एवं परिवार के सदस्यों की संख्या साझा करें।', 'Your presence would bring us great joy. Please share your preferred time and number of family members.')}
        </p>
        ...
        <form onSubmit={handleSendWhatsApp} className="mt-5 space-y-4 font-devanagari">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#e2b866] mb-1.5">
              {t('आपका शुभ नाम / परिवार का नाम *', 'Your name / family name *')}
            </label>
            <input
              type="text"
              required
              ...
              placeholder={t('उदा. रमेश कुमार एवं परिवार', 'e.g. Ramesh Kumar & family')}
          ...
          <div>
            <label className="block text-xs font-semibold text-[#e2b866] mb-1.5 flex items-center justify-between">
              <span className="flex items-center gap-1.5">
                ...
                <span>{t('कितने सदस्य पधारेंगे?', 'How many guests will attend?')}</span>
              </span>
              <span className="text-[#faf5ee] font-bold text-sm">{guestCount} {t('सदस्य', 'guests')}</span>
            </label>
            <div className="grid grid-cols-6 gap-2">
              {[1, 2, 3, 4, 5, 6].map((num) => (
                ...
                >
                  {language === 'en' ? (num === 6 ? '6+' : num) : num === 6 ? '६+' : ['१', '२', '३', '४', '५'][num - 1]}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#e2b866] mb-1.5 flex items-center gap-1.5">
              ...
              <span>{t('दर्शन / आरती का पसंदीदा समय', 'Preferred darshan / aarti time')}</span>
            </label>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#e2b866] mb-1.5">
              {t('अतिरिक्त संदेश (वैकल्पिक)', 'Additional message (optional)')}
            </label>
            <input
              type="text"
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder={t('उदा. हम शाम की आरती में सम्मिलित होंगे', 'e.g. We will join the evening aarti')}
          ...
          <div className="pt-3">
            ...
            >
              ...
              <span>{t('व्हाट्सएप पर पुष्टि भेजें (WhatsApp RSVP)', 'Send confirmation on WhatsApp')}</span>
            </button>
            <p className="text-center text-[11px] text-[#a89e92] mt-2">
              {t('यह सीधे मेजबान परिवार को व्हाट्सएप पर संदेश भेजेगा', 'This will send a message directly to the host family on WhatsApp')}
            </p>
          </div>
        </form>
      </div>
