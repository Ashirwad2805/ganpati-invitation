...
class SacredAudioEngine {
  ...
  private initContext() {
    ...
    if (this.ctx.state === 'suspended') {
      void this.ctx.resume().catch((error) => console.warn('Audio resume warning:', error));
    }
  }
  ...
  public startAmbient() {
    ...
    try {
      ...
      frequencies.forEach((freq, idx) => {
        ...
        const individualGain = 0.12 / (idx + 1);
      ...
      const melody = [392.0, 392.0, 466.16, 523.25, 466.16, 392.0, 349.23, 392.0];
      let melodyIndex = 0;
      const playNote = () => {
        if (!this.ctx || !this.masterGain || !this.isPlaying) return;
        ...
        note.type = 'triangle';
        note.frequency.setValueAtTime(melody[melodyIndex], noteNow);
        noteGain.gain.setValueAtTime(0.0001, noteNow);
        noteGain.gain.exponentialRampToValueAtTime(0.12, noteNow + 0.025);
        noteGain.gain.exponentialRampToValueAtTime(0.0001, noteNow + 0.72);
        note.connect(noteGain);
        noteGain.connect(this.masterGain);
        note.start(noteNow);
        note.stop(noteNow + 0.8);
        melodyIndex = (melodyIndex + 1) % melody.length;
      };
      playNote();
      this.melodyTimer = window.setInterval(playNote, 820);
    } catch (e) {
